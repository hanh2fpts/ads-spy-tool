...
// Extract the first non-Google destination URL embedded in ad creative HTML.
function extractUrlFromAdHtml(content) {
  const html = content?.["3"]?.["2"] || '';
  if (!html) return null;
  for (const m of html.matchAll(/href="(https?:\/\/[^"]+)"/g)) {
    try {
      const url = new URL(m[1]);
      if (!GOOGLE_HOST_RE.test(url.hostname)) return m[1];
    } catch (_) {}
  }
  return null;
}
...
function parse(raw, enrichments = new Map()) {
  ...
  return creatives.map((c) => {
    ...
    const homepageUrl = (creativeId && enrichments.get(creativeId)) || extractUrlFromAdHtml(c["3"]) || null;
